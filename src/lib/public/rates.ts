...
// servicio con la tarifa VIGENTE del motor cuando hay match inequívoco.
//
...
//    servicios[].title (6/6 medido s51). Names DUPLICADOS en el tenant →
//    ese name se descarta entero (nunca elección no-determinística).
//  - Solo se PISA un precio que el content YA muestra (price_ars presente):
//    jamás se agrega precio a un servicio sin precio — la FORMA del render
...
type ServicioLike = { title: string; price_ars?: number };
...
/**
 * Pisa price_ars de cada servicio cuyo title matchea una tarifa vigente.
 * Map vacío → devuelve EL MISMO objeto sin tocar nada (no-op por identidad:
 * el camino de Hakuna y de cualquier tenant sin motor). Muta in-place el
 * clon que devuelve zod.parse (verificado Pass-1: parse devuelve objeto
 * plano nuevo, no frozen). price_regular_ars NUNCA se toca: si el vigente
 * supera al regular, la oferta tachada desaparece sola (offerPct → 0),
 * comportamiento legal-safe ya existente en el template.
 */
export function applyCurrentRates<T extends { servicios?: ServicioLike[] }>(
  content: T,
  rates: Map<string, number>,
): T {
  if (rates.size === 0) return content;
  const servicios = content.servicios;
  if (!servicios || servicios.length === 0) return content;

  const unmatched: string[] = [];
  for (const s of servicios) {
    const price = rates.get(s.title);
    if (price == null) continue;
    if (s.price_ars == null) {
      // El content no muestra precio para este servicio: no agregamos uno
  ...
  for (const name of rates.keys()) {
    if (!servicios.some((s) => s.title === name)) unmatched.push(name);
  }
  if (unmatched.length > 0) {
    // Excursión del motor sin servicio correspondiente en content_json:
