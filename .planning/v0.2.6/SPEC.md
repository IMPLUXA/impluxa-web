...
> **Status**: SPEC merged sesion 15. Subsequent work: 5B.8 ADR-0010 DONE (`a9ff1e5`) → 5B.9 integration tests DONE (`8f74946`) → 5B.10 Diff Two-Pass cold DONE (3 cold agents `a7b7c117218eba2c1` / `aa6f3893b8a3b05cd` / `a737a3cc9b57963d4`, 4 HIGH fixes applied s15 cierre) → ~~5B.11 ASK CEO mark PR #6 Ready~~ **SLIP A s16** (capacity restante s15 ~1-3h activo; coincide con 5B.12 slip; cierre conjunto en s16 mas honesto que rush con CEO presence-dependiente; `hakuna_live=false` hace slip riesgo nulo) → ~~5B.12 Sec 2.d merge~~ **SLIP ACEPTADO A s16** (OQ-PM-1 resuelta s15 post primer pass).
...
| I7  | `audit_dedup` GC cron `audit_dedup_gc` runs daily at 03:00 UTC (migration `001:69-71` schedule `'0 3 * * *'`); retains 7 days; COMMIT-between-batches at migration `001:93` is legal on PG 17.6 Supabase managed (DBO-H5 empirical verified sesion 15 via preview `llyexugyuwwdqfarumbj`).                                                            | DBO primer §1.2, DBO cold DBO-H5 NEW, empirical s15                  | RUNBOOK-5B.md §5 documents test result. Closure at pg_cron enable in prod.                                                                 |
...
| 9   | Caso #8 fresh dossier completo archivado as `.md` artifacts (per lesson `dossier-two-pass-extended-no-archivado-pre-cierre`) | 8 dedicated dossier `.md` files in `D:\impluxa-web\.planning\v0.2.6\` satisfy the "artefacto .md aparte" requirement: 3 PASS-1 (`W1.T1-5B-SPEC-PASS-1-{BA,DBO,PM}.md`) + 3 PASS-2 (`W1.T1-5B-SPEC-PASS-2-{BA,DBO,PM}.md`) + 2 REREVIEW (`W1.T1-5B-C-H2-REREVIEW-{BA,SE}-FRESH.md`) + 3 5B.10 cold (`W1.T1-5B.10-DIFF-TWO-PASS-COLD-{BA,DBO,PM}.md`) = 11 total. SPEC §10 + §10.10 References serve as consolidated synthesis with citations. Path B chosen sesion 15 over a separate `caso8-dossier-completo.md` because §10 already does the consolidation work and the 8 files are already `.md aparte` (not dispersed in hot.md / Telegram / observaciones partial — the original problem the lesson addresses). |
...
| R7  | Caso #8 fresh dossier completo NOT archived pre-cierre s15 (lesson `dossier-two-pass-extended-no-archivado-pre-cierre`) | CLOSED (s15 cierre)            | RESOLVED via Path B sesion 15: 8 dossier `.md aparte` already in `.planning/v0.2.6/` satisfy the lesson; SPEC §10 is consolidated synthesis; SPEC §10.7 gate 9 rewording acknowledges this. See §10.7 gate 9 + ADR-0010 D10.x references. |
