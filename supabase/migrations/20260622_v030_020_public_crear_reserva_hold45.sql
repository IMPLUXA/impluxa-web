-- v030_020 — F4: subir el hold anon de 30 a 45 minutos (ventana de pago MP). Decision CEO s59.
--
-- CREATE OR REPLACE de public_crear_reserva (v030_018) BYTE-IDENTICO salvo la constante
-- v_hold_minutes (30 -> 45). El anon necesita una ventana mas larga para completar el
-- Checkout Pro de MercadoPago (login + tarjeta). El cupo se libera solo cuando el hold vence
-- (el predicado anti-oversell ya cuenta pre_reserva solo WHERE hold_expires_at > now()), asi
-- que subir el hold a 45 min es consistente sin tocar barredores ni el motor agency.
--
-- NO re-crea la columna/indice de idempotencia (ya existen desde v030_018). Solo reemplaza la
-- funcion. El motor agency_crear_reserva (v030_012) queda INTACTO (cero CREATE OR REPLACE sobre el).
-- _down restaura el body con := 30.
...
declare
  v_tenant       uuid;
  v_hold_minutes constant integer := 45;   -- C-1/F4: hold 45 min anon (ventana de pago MP; decision CEO s59)
...
-- GRANT SOLO service_role. **NO anon, NO authenticated.** (igual que v030_018). El flujo público pasa
-- SIEMPRE por el endpoint Next.js server-side (rate-limit + Turnstile + honeypot) y desde ahí
-- service-role llama la RPC. El motor agency_crear_reserva queda INTACTO.
